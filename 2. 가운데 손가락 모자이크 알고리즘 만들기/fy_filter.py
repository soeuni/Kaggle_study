...
max_num_hands = 5 # 최대 몇 개의 손을 인식할 수 있느냐
gesture = { # fy 클래스 정의
    0: 'fist', 1: 'one', 2: 'two', 3: 'three', 4: 'four', 5: 'five',
    6: 'six', 7: 'rock', 8: 'spiderman', 9: 'yeah', 10: 'ok', 11: 'fy'
}

# MediaPipe hands model (첫번째 모델: 손 감지 모델)
...
hands = mp_hands.Hands(# --> 손가락 detection 모듈 초기화
    ...
    min_detection_confidence=0.5, # 0.5로 해두는게 좋음 (왜? )
    min_tracking_confidence=0.5)

# Gesture recognition model (두번째 모델: 제스처 인식 모델)
file = np.genfromtxt('data/gesture_train_fy.csv', delimiter=',') # 모아둔 data (손가락의 각도들과 마지막에 label 값 저장되어 있는 파일)
angle = file[:,:-1].astype(np.float32)
label = file[:, -1].astype(np.float32)
knn = cv2.ml.KNearest_create() # opencv의 K-Nearest Neighbors 알고리즘(K-최근접 알고리즘)
knn.train(angle, cv2.ml.ROW_SAMPLE, label) # 학습 시키기

cap = cv2.VideoCapture(0) # 0번 카메라 사용, 웹캠의 이미지 읽어옴
